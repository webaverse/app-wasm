...
float* generateTerrain(
    ...
    float *biomeBuffer = (float*)malloc(maxVertexCount * 6 * sizeof(float));
    ...
    for (int i = 0; i < chunkCount; i++) {
        for (int j = 0; j < chunkCount; j++) {
            for (int k = 0; k < chunkCount; k++) {
                ...
                memcpy(biomeBuffer + totalVertexCount * 6, &(biomes.front()), biomes.size() * sizeof(VertexBiome));
...
int* generateAndAllocateChunk(
    ...
    memcpy(biomeBuffer + vertexOffset * 6, &(biomes.front()), biomes.size() * sizeof(VertexBiome));
...
void march(
	...
	std::map<std::string, int> & vertexDic, int & index, float biome[6]
) {
            ...
            for (int i = 0; i < 3; i++) {
            	...
            	if (vertexDic.find(vInx) != vertexDic.end()) {
                    ...
                    if (
                        ...
                        biomes.push_back({biome[0], biome[1], biome[2], biome[3], biome[4], biome[5]});
